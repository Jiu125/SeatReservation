...
### 짧은 소개
현재 프로그램은 자리를 예약하는 기본적인 프로그램을 보여주고 있습니다.
프로그램은 자리를 예약하고 취소하는 방법을 생각하다가 물리적인 유무를 확인할 수 없는 한계점을 시간을 지정하여 해결하였습니다.  
프로토타이므로 추후에는 35분으로 지연 시간을 올릴 예정입니다. 하지만, 프로토타입이므로 현재는 **3초**로 시간을 설정하였습니다.  
사용자가 가독성을 높이고 싶어 '결제창'을 움직여서 자신이 선택한 좌석을 볼 수 있게 해두었습니다.  
하지만, 이 프로그램의 한계점은 결제창의 버튼을 눌러야 메인창의 '구매' 버튼이 다시 활성화된다는 점입니다.
이 문제점은 추후에 수정할 '예외 처리' 사항으로 넣었습니다.  
이 프로그램은 앞으로 식당을 이용하는 학생들과 다른 사람들의 편의를 위해서 편의성을 높이는 프로그램으로 발전시킬 것입니다.
